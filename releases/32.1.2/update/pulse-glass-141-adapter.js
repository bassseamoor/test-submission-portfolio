...
const C={
  ...
  live:true,contract:'moor.render/1',adapterId:'pulse-glass-141.webgl2',implementationVersion:'PulseGlass141/0.1.2',
...
class Session{
  ...
  stop(){try{this.legacy.render=this.originalRender}catch{}this.restore();try{for(const b of this.buffers.values())for(const x of [b.pos,b.norm,b.idx])if(x)this.gl.deleteBuffer(x);this.gl.deleteProgram(this.prog)}catch{}this.ctx.renderService=this.previousService;this.ctx.activeAdapter=this.previousAdapter;if(this.previousGetTelemetry)this.ctx.getRenderTelemetry=this.previousGetTelemetry;else delete this.ctx.getRenderTelemetry;if(this.w.MOORRenderService===this.service)this.w.MOORRenderService=this.previousGlobalService||null}
}
